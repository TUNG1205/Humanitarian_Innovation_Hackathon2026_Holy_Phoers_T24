    ...
    <div style={{ display: "grid", gap: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 8 }}>
          {EMERGENCY_NUMBERS.map((e, i) => (
            <div key={i} style={{
              ...
              <div>
                <div style={{ fontFamily: mono, fontSize: 9, color: "#3d4a63", letterSpacing: "0.12em", marginBottom: 5 }}>{e.cat}</div>
                <div style={{ fontFamily: sans, fontSize: 12, color: "#94a3b8", marginBottom: 5 }}>{e.svc}</div>
                <div style={{ fontFamily: condensed, fontSize: 22, fontWeight: 700, color: e.live ? "#22c55e" : "#64748b", letterSpacing: "0.06em" }}>{e.num}</div>
              </div>
              <div style={{
                fontFamily: mono, fontSize: 9, letterSpacing: "0.1em", padding: "3px 8px", borderRadius: radiusSm,
                border: `1px solid ${e.live ? "#22c55e" : "#3d4a63"}`,
                color: e.live ? "#22c55e" : "#64748b",
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
          <div style={{ fontFamily: condensed, fontSize: 13, fontWeight: 700, letterSpacing: "0.16em", color: "#4b5875" }}>
        ...
        <div style={{ fontFamily: sans, fontSize: 12, color: "#64748b", marginBottom: 12 }}>
        ...
        <div style={{ display: "grid", gap: 5 }}>
          {OFFLINE_GUIDES.map((guide, i) => (
            <div key={i} style={{ background: "#131a26", border: cardBorder, borderRadius: radius }}>
              <button onClick={() => setOpenGuide(openGuide === i ? null : i)} style={{
                ...
                <ChevronRight size={15} color="#4b5875" style={{ transform: openGuide === i ? "rotate(90deg)" : "none", transition: "transform 0.2s" }} />
              </button>
              {openGuide === i && (
                <div style={{ padding: "0 20px 16px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                  <div style={{ fontFamily: sans, fontSize: 13, color: "#94a3b8", lineHeight: 1.8, paddingTop: 13 }}>
                    {guide.body}
                  </div>
                </div>
              )}
