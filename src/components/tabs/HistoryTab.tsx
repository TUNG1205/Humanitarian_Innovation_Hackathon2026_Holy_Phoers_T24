...
import { mono, sans, cardBorder, radius, radiusSm, accent, cardBg, cardShadow } from "../../lib/theme";
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        {types.map(t => (
          <button key={t} onClick={() => setFilter(t)} style={{
            ...
            color: filter === t ? accent : "#a3b1c9",
          }}>{t}</button>
        ))}
      </div>

      <div style={{ background: cardBg, border: cardBorder, borderRadius: radius, boxShadow: cardShadow, overflow: "hidden" }}>
        <div style={{ display: "grid", gridTemplateColumns: "70px 1fr 110px 150px 110px 90px", padding: "12px 20px", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
          {["YEAR", "EVENT", "TYPE", "AFFECTED", "DAMAGE", "SEVERITY"].map(h => (
            <div key={h} style={{ fontFamily: mono, fontSize: 9, color: "#8492ab", letterSpacing: "0.14em" }}>{h}</div>
          ))}
        </div>
        {shown.map((d, i) => (
          <div key={i} style={{
            ...
            <div style={{ fontFamily: mono, fontSize: 11, color: "#a3b1c9" }}>{d.year}</div>
            ...
            <div style={{ fontFamily: mono, fontSize: 11, color: "#a3b1c9" }}>{d.affected}</div>
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 10 }}>
          {[
            ...
            <div key={i} style={{ background: cardBg, border: cardBorder, borderRadius: radius, boxShadow: cardShadow, padding: "14px 16px" }}>
              ...
              {([["FLOOD", z.flood], ["HIGH WIND", z.wind], ["STORM SURGE", z.surge]] as [string,number][]).map(([label, val]) => (
                <div key={label} style={{ marginBottom: 7 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                    <span style={{ fontFamily: mono, fontSize: 9, color: "#8492ab" }}>{label}</span>
