...
import { mono, condensed, sans, cardBorder, radius } from "../../lib/theme";
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {RECOVERY_PLANS.map((r, i) => (
            <button key={i} onClick={() => setSel(i)} style={{
              background: sel === i ? "#131a26" : "transparent",
              ...
              <div>
                ...
                <div style={{ fontFamily: mono, fontSize: 9, color: "#64748b", marginTop: 2 }}>{r.phase}</div>
              </div>
            </button>
          ))}
        </div>

        <div style={{ background: "#131a26", border: cardBorder, borderRadius: radius, padding: "24px 28px" }}>
          <div style={{ marginBottom: 22 }}>
            ...
            <div style={{ fontFamily: mono, fontSize: 10, color: "#64748b", marginTop: 6 }}>{p.phase}</div>
          </div>

          <div style={{ marginBottom: 22 }}>
            <div style={{ fontFamily: mono, fontSize: 9, color: "#3d4a63", letterSpacing: "0.14em", marginBottom: 14 }}>ACTION STEPS</div>
            {p.steps.map((step, i) => (
              <div key={i} style={{ display: "flex", gap: 16, marginBottom: 12, alignItems: "flex-start" }}>
                ...
                <div style={{ fontFamily: sans, fontSize: 13, color: "#94a3b8", lineHeight: 1.65 }}>{step}</div>
              </div>
            ))}
          </div>

          <div>
            <div style={{ fontFamily: mono, fontSize: 9, color: "#3d4a63", letterSpacing: "0.14em", marginBottom: 10 }}>RESOURCES & REFERENCES</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(210px, 1fr))", gap: 10 }}>
          {[
            ...
            <div key={i} style={{ background: "#131a26", border: cardBorder, borderRadius: radius, padding: "16px 18px" }}>
              ...
              <div style={{ fontFamily: sans, fontSize: 12, color: "#8a97ab", lineHeight: 1.65 }}>{item.desc}</div>
            </div>
          ))}
        </div>
      </div>
